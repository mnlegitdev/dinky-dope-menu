# Technical Handoff — Dinky Dope Menu
...
1. **`scraper.py`** — Pulls live inventory from the Sweed POS system.
   ...
   - **Product identity** (`product_key()`): each product is keyed on Sweed's own `id` field from the raw API response (`sw{id}`), not on its name/brand/category text. This matters because Sweed relabels products often (pack-size wording, punctuation) — a text-based key would treat every relabel as a brand-new product and falsely reset `first_seen`, which is exactly what happened before 2026-07-15 (see `server-runbook` incident). Only Strategy 3 (DOM fallback) still falls back to the old name+brand+category text key, since the DOM has no access to Sweed's internal id — a rename during a DOM-fallback period could still trigger a false "new" badge. Known, currently-unresolved edge case; Strategy 3 is rare in practice.
   - **If the key scheme is ever changed again**: `docs/strains_enriched.json` (see next step) is keyed the same way — update both together. Missing this the first time caused `enrich_strains.py` to treat the entire menu as unenriched and start a full re-enrichment run via the Claude API mid-deploy (on the sibling `legit-cannabis-south-metro-menu` repo; caught and fixed before deploying here).
...
2. **`enrich_strains.py`** — For every product in `products.json` that doesn't yet have an entry in `docs/strains_enriched.json`, it calls the Claude API twice:
   ...
   - Once to rate the strain 0–10 for each of the 9 mood categories, based on its COA terpene order
